...
videoUploadGuestJS =
  ...
  <> "function msg(m){var b=document.getElementById('video-upload-open');if(!b)return;var p=document.getElementById('video-login-message');if(!p){p=document.createElement('p');p.id='video-login-message';p.className='video-login-message';b.parentNode.insertBefore(p,b.nextSibling);}p.textContent=m||'';}"
  <> "function status(m,e){var s=document.getElementById('video-upload-status');if(s){s.textContent=m||'';s.classList.toggle('is-error',!!e);}}"
  <> "function mark(v){window.__weddingRsvpIdentified=!!v;var b=document.getElementById('video-upload-open');if(b){b.classList.toggle('is-disabled',!v);b.setAttribute('aria-disabled',v?'false':'true');}if(v)msg('');}"
  ...
  <> "function check(){return fetch('/api/rsvp/me',{credentials:'same-origin'}).then(function(r){mark(r.ok);return r.ok;}).catch(function(){mark(false);return false;});}"
  <> "function login(){var c=code();if(!c||window.__weddingInviteLoginTried)return;window.__weddingInviteLoginTried=1;fetch('/api/rsvp/login',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({code:c})}).then(function(r){if(r.ok){mark(true);}else{mark(false);}}).catch(function(){mark(false);});}"
  <> "function watchRsvp(){if(window.__weddingRsvpSubmitWatchReady)return;window.__weddingRsvpSubmitWatchReady=1;var open=XMLHttpRequest.prototype.open;XMLHttpRequest.prototype.open=function(m,u){this.__weddingRsvpUrl=String(u||'');return open.apply(this,arguments);};var send=XMLHttpRequest.prototype.send;XMLHttpRequest.prototype.send=function(){if(this.__weddingRsvpUrl==='/api/rsvp')this.addEventListener('load',function(){if(this.status>=200&&this.status<400){mark(true);msg('');}});return send.apply(this,arguments);};}"
  <> "function upload(){var f=document.getElementById('video-upload-form');if(!f||f.dataset.guestReady)return;f.dataset.guestReady='1';f.addEventListener('submit',function(e){e.preventDefault();if(!window.__weddingRsvpIdentified){fail('Confirma tu RSVP para poder subir un video.');return;}var file=document.getElementById('video-upload-file');if(!file||!file.files||!file.files.length){fail('Selecciona un video.');return;}var data=new FormData(f);data.set('file',file.files[0],file.files[0].name);var xhr=new XMLHttpRequest();xhr.open('POST','/api/videos');xhr.withCredentials=true;xhr.upload.onprogress=function(ev){if(ev.lengthComputable)progress((ev.loaded/ev.total)*100,true);};xhr.onload=function(){if(xhr.status>=200&&xhr.status<300){progress(100,true);status('Video recibido. Gracias por enviarlo.',false);f.reset();}else{fail(clean(xhr.responseText)||'No se pudo subir el video. Intentalo de nuevo.');}};xhr.onerror=function(){fail('No se pudo subir el video. Intentalo de nuevo.');};progress(0,true);status('Subiendo video...',false);xhr.send(data);});}"
  <> "function start(){watchRsvp();upload();check().then(function(ok){if(!ok)login();});}"
